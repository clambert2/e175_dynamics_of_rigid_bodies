function [t,VAR,Output] = hw_11_18_19
%===========================================================================
% File: hw_11_18_19.m created Nov 06 2025 by MotionGenesis 6.5.
...
phi                             =  20;                     % degrees             Initial Value
qC                              =  0;                      % UNITS               Initial Value
theta                           =  0;                      % degrees             Initial Value
phiDt                           =  0;                      % UNITS               Initial Value
qCDt                            =  300;                    % rpm                 Initial Value
thetaDt                         =  0;                      % UNITS               Initial Value

tInitial                        =  0.0;                    % second              Initial Time
tFinal                          =  4;                      % second              Final Time
...
DEGtoRAD = pi / 180.0;
RADtoDEG = 180.0 / pi;
phi = phi * DEGtoRAD;                                      %  Converted from degrees 
theta = theta * DEGtoRAD;                                  %  Converted from degrees 
qCDt = qCDt * 0.1047197551196598;                          %  Converted from rpm 
...
thetaDDt = -2*phiDt*(r^2*qCDt+4*L^2*cos(phi)*thetaDt)/((r^2+4*L^2)*sin(phi));
phiDDt = sin(phi)*(4*g*L+4*L^2*cos(phi)*thetaDt^2+r^2*thetaDt*(2*qCDt-cos(phi)*thetaDt))/(r^2+4*L^2);
qCDDt = -phiDt*((2*r^2+(4*L^2-r^2)*sin(phi)^2)*thetaDt+2*cos(phi)*(r^2*qCDt+4*L^2*cos(phi)*thetaDt-r^2*cos(phi)*thetaDt))/((r^2+4*L^2)*  ...
sin(phi));
...
if( isempty(Output) ),
   if( ~isempty(FileIdentifier) ),
      ...
      fprintf( 1, '\n Output is in the files hw_11_18_19.i  (i=1,2)\n\n' );
   end
...
if( isempty(hasHeaderInformationBeenWritten) ),
   ...
   if( shouldPrintToFile && isempty(FileIdentifier) ),
      ...
      FileIdentifier(1) = fopen('hw_11_18_19.1', 'wt');   if( FileIdentifier(1) == -1 ), error('Error: unable to open file hw_11_18_19.1'); end
      fprintf(FileIdentifier(1), '%% FILE: hw_11_18_19.1\n%%\n' );
      fprintf(FileIdentifier(1), '%%       t            theta\n' );
      fprintf(FileIdentifier(1), '%%   (second)       (degrees)\n\n' );
      FileIdentifier(2) = fopen('hw_11_18_19.2', 'wt');   if( FileIdentifier(2) == -1 ), error('Error: unable to open file hw_11_18_19.2'); end
      fprintf(FileIdentifier(2), '%% FILE: hw_11_18_19.2\n%%\n' );
      fprintf(FileIdentifier(2), '%%       t             phi\n' );
...
%===========================================================================
figure;
data = load( 'hw_11_18_19.1' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
title('Precession: \theta^{\circ} versus t (sec)')
xlabel('t (sec)');   ylabel('\theta^{\circ}');   % title('Some plot title');
clear data;

figure;
data = load( 'hw_11_18_19.2' ); 
plot( data(:,1),data(:,2),'-b', 'LineWidth',3 );
title('Nutation: \phi^{\circ} versus t (sec)')
xlabel('t (sec)');   ylabel('\phi^{\circ}');   % title('Some plot title');
clear data;
end
...
%====================================
end    % End of function hw_11_18_19
%====================================
